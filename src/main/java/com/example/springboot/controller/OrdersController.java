...
import com.example.springboot.service.ICartService;import com.example.springboot.service.IBookService;
...
public class OrdersController {
    ...
    public Result save(@RequestBody Orders orders) {
        if (orders.getId() == null) {
            Book book = bookService.getById(orders.getBookId());
            if (book.getInventory()-orders.getNum()<0){
            ...
            orders.setPrice(new BigDecimal(orders.getNum()).multiply(book.getPrice()));
            orders.setTime(DateUtil.now());
            orders.setImg(book.getImg());
            ...
            book.setInventory(book.getInventory()-orders.getNum());
            book.setSales(book.getSales()+orders.getNum());
            bookService.updateById(book);
            recordBehavior(orders.getBookId(), "order", 5);
        }
    ...
    public Result fromCart(@RequestBody List<Cart> carts, @PathVariable Integer addressId) {
        for (Cart cart : carts) {
            ...
            // 检查库存
            if (book.getInventory() - order.getNum() < 0) {
                return Result.error("605","图书库存不足");
            }
            book.setInventory(book.getInventory()-order.getNum());
            book.setSales(book.getSales()+order.getNum());
            bookService.updateById(book);
            order.setImg(book.getImg());
            order.setName(book.getName());
            order.setPrice(new BigDecimal(cart.getNum()).multiply(book.getPrice()));
            order.setStatus("待支付");
            ...
            recordBehavior(cart.getBookId(), "order", 5);
        }
    ...
    public Result findGoodComment(@PathVariable Integer id) {
        ...
        wrapper.eq(Orders::getBookId,id);
    ...
    public Result findPage(@RequestParam Integer pageNum,
        ...
        //如果不是管理员，只显示用户自己的订单信息
        if (!StrUtil.equals(account.getRole(),"ROLE_ADMIN")){
            queryWrapper.eq(Orders::getUserId,account.getId());
        }
