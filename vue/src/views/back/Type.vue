...
// 加载数据
const load = () => {
  request.get("/book/category/page", {
    params: {
...
const save = () => {
  ...
  request.post("/book/category", form.value).then(res => {
    if (res.code === '200') {
...
// 删除
const del = (id) => {
  request.delete("/book/category/" + id).then(res => {
    if (res.code === '200') {
...
const delBatch = () => {
  ...
  request.post("/book/category/del/batch", ids).then(res => {
    if (res.code === '200') {
